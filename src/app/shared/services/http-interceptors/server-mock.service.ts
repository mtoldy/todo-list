...
import { ALL_TODO, DELETE_TODO, SAVE_TODO, TOGGLE_TODO } from 'src/app/shared/constants/urls';
...
export class ServerMockService implements HttpInterceptor {
  ...
  intercept(
    ...
    switch (true) {
      ...
      case req.url.startsWith(TOGGLE_TODO): {
        return this.sendResponse({...req.body, complete: !req.body.complete}, true);
      }
  ...
  sendResponse(body: Todo | Todo[], trottling: boolean): Observable<HttpResponse<Todo | Todo[]>> {
    return of(new HttpResponse({ status: 200, body })).pipe(
        delay(trottling ? Math.random() * 1000 : 0),
        switchMap( obs => trottling && Math.random() > 0.6 ? throwError(() => new Error('Weird network error ;)')) : of(obs) )
    );
  }
}
